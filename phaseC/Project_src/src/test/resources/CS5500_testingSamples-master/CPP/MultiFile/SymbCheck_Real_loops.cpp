...
const SymbCheck_Real SymbCheck_Real::operator * (const SymbCheck_Real& scr) const {
    std::stringstream sstm("");
    int i = 0;
    while( i < vars.size()) { // for every element in "this" expression
        if (i != 0)
            sstm << "+";
...
void SymbCheck_Real::print() const {
    std::cout << "\n";
    int i = 0;
    while( i < vars.size()) { // every element in vars is part of a sum
        if (i != 0)
            std::cout << " + ";
...
bool contains(const std::vector<std::string>& v, const std::string& check, int& index) {
    ...
    for (int i = 0; i < v.size(); ++ i) {
        ...
        int k = 0;
        while( k < checkSet.size()) { // for every element in the product check
...
std::vector<double> colRemCoeff(std::vector<std::string>& v) {
    ...
    int i = 0;
    while (i < v.size()) { // for every entry in the array of strings
